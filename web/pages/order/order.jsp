		...
		<c:if test="${not empty requestScope.user_orders}">
			<c:forEach items="${requestScope.user_orders}" var="user_orders">
				<tr>
						<%--格式化输出日期--%>
					<td>
						<fmt:formatDate value="${user_orders.createTime}" pattern="yyyy.MM.dd"/>
					</td>
					<td>${user_orders.price}</td>
					<td>
						<c:if test="${user_orders.status eq 0}">${user_orders.orderId}</c:if>
					...
					<td>
						...
						<c:if test="${user_orders.status eq 1}">
							<a class="ReceivingGoods"
							   href="orderServlet?action=receiveOrder&orderId=${user_orders.orderId}">确认收货</a>
						</c:if>
							<%--否则无法点击，显示对应状态信息。--%>
