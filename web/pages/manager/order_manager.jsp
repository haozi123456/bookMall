...
<%--					<fmt:parseDate value="${orders.createTime}" var="releaseTime" pattern="yyyy-MM-dd"/>--%>
<%--					<fmt:formatDate value="${releaseTime}" pattern="yyyy-MM-dd"/>--%>
				...
				<td><a href="orderServlet?action=showOrderByOrderId&orderId=${orders.orderId}">查看详情</a></td>
				<td>
					<c:if test="${orders.status eq 0}">
						<a class="DeliverGoods"
						   href="orderServlet?action=sendOrder&orderId=${orders.orderId}">发货</a>
					</c:if>
